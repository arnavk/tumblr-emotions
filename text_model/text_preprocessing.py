...
def preprocess_df(text_dir, emb_dir, filename, emb_name, emotions, post_size):
    ...
    # columns = [
    #     'num', 'id', 'post_url', 'date', 'liked', 'type', 'timestamp',
    #     'tags', 'photo', 'caption', 'search_query', 'img_file_name'
    # ]
    df_all = pd.DataFrame(columns=columns)
    for emotion in emotions:
        path = os.path.join('data', emotion + '.csv')
        # Get all posts for that emotion
...
def preprocess_one_df(vocabulary, embedding, emotion, post_size):
    ...
    # path = os.path.join('data', emotion + '.csv')
    path = os.path.join('data', 'shuffled_train.csv')
    df_emotion = _df_with_hashtag_in_post(
        pd.read_csv(path, encoding='utf-8'), emotion)
    # All posts with emotion in it
    vocab_set = set(vocabulary)
    mask = df_emotion['caption'].map(lambda x: _is_valid_text(x, vocab_set))
    df_emotion =  df_emotion.loc[mask, :].reset_index(drop=True)

    emotions = ['happy', 'sad', 'angry', 'scared', 'disgusted', 'surprised']
    # Map text to ids
    df_emotion['text_list'], df_emotion['text_len'] = zip(*df_emotion['caption'].map(lambda x:
        _paragraph_to_ids(x, word_to_id, post_size, emotions)))
